...
// H8 から直接アクセスできるのはレジスタのみ
// RTL8019AS は SAPROM と呼ばれるメモリ領域を持っているが、ここにアクセスするには RDMA を使う
...
static int write_data(int addr, int size, char *buf)
{
  ...
  *NE2000_CR    = NE2000_CR_P0 | NE2000_CR_RD_WRITE | NE2000_CR_STA;
  for (i = 0; i < size; i++) {
    // x86 Linux のドライバだと outb などを使っている
    // x86 はデバイスへのアクセスに IO ポートを使うため
    // 早くアクセスしすぎると動作が間に合わなくなるデバイスもあるので、
    // outb_p のように _p 付きのものが用意されている

    // ウェイトも入れずにどんどん書いていいのか？
    // RSAR で指定した値が徐々にずれていくのかも？
    // そのあたりの動作が明確に書かれた仕様が見つからない
    *RTL8019_RDMAP = buf[i];
  }
...
int ether_init(int index)
{
  ...
  *NE2000_ISR    = 0xff;
  ...
  // SAPROM の 0 番地から 12 バイト読み込む(最初に MAC アドレスが入っている？)
  read_data(0, 12, t2);
  puts("MAC: ");
  for (i = 0; i < 6; i++) {
    macaddr[i] = t2[i * 2];
